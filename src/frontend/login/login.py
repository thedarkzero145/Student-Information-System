...
def open_login_window(window):
    ...
    def onSubmit():
        ...
        is_user_valid = user_validation(username)
        is_pass_valid = pass_validation(password)
    ...
    # Side Frame of Login
    side_frame = Frame(window, width=430)
    ...
    bg_label_image =  Label(side_center_frame, image=bg_logo)
    ...
    Label(side_center_frame,
          ...
          font=(FONT_DEFAULT_NAME, 30,
                "bold"), wraplength=350,  foreground="#002961").pack(fill="both")

    Label(side_center_frame,
          text="STUDENT INFORMATION SYSTEM",
          font=(FONT_DEFAULT_NAME, 10, "bold"), foreground="#002961").pack(fill="both")

    main_frame = Frame(window, style=CUSTOM_BACKGROUND_NAME)
    main_frame.pack(fill="both", expand=True)

    center_content = Frame(main_frame, style=CUSTOM_BACKGROUND_NAME)
    ...
    user_input = Entry(center_content, width=200,
                       ...
                       style="BG_ENTRY.TEntry"
                       )
    user_input.pack(ipady=8, pady=(4, 0))

    user_error_label = Label(center_content, bootstyle="danger", background=CUSTOM_BACKGROUND_COLOR)
    user_error_label.pack(anchor="w")

    # Password Label
    password_label = Label(center_content, text="Password", style="BG_LABEL.TLabel")
    ...
    password_error_label = Label(center_content, bootstyle="danger", background=CUSTOM_BACKGROUND_COLOR)
    password_error_label.pack(anchor="w")

    remember_me_frame = Frame(center_content, style=CUSTOM_BACKGROUND_NAME)
    remember_me_frame.pack(anchor="w", pady=8)


    remember_me_checkbox = Checkbutton(remember_me_frame, style="BG_CHECKBOX.TCheckbutton")
    remember_me_checkbox.pack(side=LEFT)

    remember_me_label = Label(remember_me_frame, text="Remember me", style="BG_LABEL.TLabel")
    remember_me_label.pack(side=LEFT)

    button = Button(center_content, bootstyle="primary",
                    text="Login")
